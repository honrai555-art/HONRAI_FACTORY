...
try {
    ...
    if (-not $unityProjectPath -or $unityProjectPath -eq "") {
        ...
        $unityProjects = Join-Path $projectRoot "unityprojects"
        
        if (Test-Path $unityProjects) {
            $projectDirs = @(Get-ChildItem -Path $unityProjects -Directory -ErrorAction SilentlyContinue)
            if ($projectDirs.Count -gt 0) {
                $unityProjectPath = $projectDirs[0].FullName
